// lib/src/features/resume_library/data/datasources/resume_library_local_data_source.dart
import 'dart:convert';
...
class ResumeLibraryLocalDataSourceImpl implements ResumeLibraryLocalDataSource {
  ...
  Future<List<ResumeSummaryDataDto>> getCachedResumes() async {
    try {
      ...
      final storedData = jsonEncode(cached);
  ...
  Future<void> cacheResumes(List<ResumeSummaryDataDto> resumes) async {
    AppLogger.logInfo(
      info: 'TO STORE DATA ${resumes.map((e) => e.toJson()).toList()}',
    );
    await _hiveLocalStorage.put(
      key: _cacheKey,
      value: resumes.map((e) => e.toJson()).toList(),
    );
  }
